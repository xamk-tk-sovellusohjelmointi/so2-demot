...
## Palvelinsovelluksen rakentuminen vaihe vaiheelta
...
Ensin tarkistetaan `count()`-metodilla, löytyykö annetulla id:llä ostos tietokannasta. Jos löytyy, haetaan se `findUnique()`-metodilla. Muussa tapauksessa heitetään virhe.
...
Ensin tarkistetaan, että annetulla id:llä löytyy ostos. Sitten validoidaan bodyn `tuote`- ja `poimittu`-kentät. `poimittu`-kenttä tarkistetaan vertaamalla suoraan `true`- ja `false`-arvoihin, koska muut arvot (kuten merkkijonot) eivät ole sallittuja boolean-kenttään. `update()` päivittää ostoksen tiedot.
...
Tässä vaiheessa palvelin toimii normaalisti selaimella ja Postmanilla. Erillinen asiakassovellus eri portissa (esim. `http://localhost:3000`) ei kuitenkaan pysty hakemaan dataa palvelimelta. Selain estää pyynnön ja konsoliin tulee CORS-virhe. **[LINKKI: ASIAKAS.md — CORS-virhe]**
...
CORS-tuen lisäämisen jälkeen asiakassovelluksen pitäisi saada vastauksia palvelimelta. **[LINKKI: ASIAKAS.md — CORS-virhe korjattu]**

# Demo 5: REST-rajapinnan testaus Postmanilla
...
Hakee kaikki tietokantaan tallennetut ajot taulukossa.

| Asetus | Arvo |
|--------|------|
| Metodi | GET |
| URL | `http://localhost:3005/api/ajopaivakirja` |
| Body | — (ei tarvita) |

Onnistunut vastaus: HTTP 200, taulukko ajoista. Jos tietokanta on tyhjä, palautuu tyhjä taulukko `[]`.
...
|--------|------|
| Metodi | GET |
| URL | `http://localhost:3005/api/ajopaivakirja/1` |
| Body | — (ei tarvita) |
...
Lisää uuden ajon tietokantaan. Pyynnön rungossa lähetetään JSON-objekti, joka sisältää ajon tiedot. `id` ja `pvm` asetetaan automaattisesti, joten niitä ei anneta.
...
Onnistunut vastaus: HTTP 201, luotu ajo-objekti `id`- ja `pvm`-kenttineen.
...
Onnistunut vastaus: HTTP 200, päivitetty ajo-objekti.
...
Poistaa ajon tietokannasta. Poistettavan ajon tunniste kirjoitetaan URL-osoitteeseen.
...
Onnistunut vastaus: HTTP 200, poistettu ajo-objekti. Vastaus sisältää poistetun tietueen tiedot, jotta pyyntöä tehneelle selviää, mikä ajo poistettiin.
